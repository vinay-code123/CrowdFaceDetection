import cv2
face_cascade = cv2.CascadeClassifier('D:\Crowd_face_detection\Crowd-face-detection-master\haarcascade_frontalface_alt_tree.xml')  # We load the cascade for the face.
 
image = cv2.imread(r"D:\Crowd_face_detection\Crowd-face-detection-master\image1.jpg")   # We load the path of the required image
grayImage = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)  # We convert the coloured image into gray image
 
faces = face_cascade.detectMultiScale(grayImage,1.0001,1)   # We apply the detectMultiScale method from the face cascade to locate one or several faces in the image.
